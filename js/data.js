...
/* ---------- 记录导出（Word）字段分类关键词 ----------
   导出时按标签把字段分入 实验材料/实验方法/实验结果 三节；k='result' 的字段固定归结果；
   顺序：先匹配结果词 → 再方法词 → 再材料词 → 都不含则归方法。改词即可调整归类。 ---------- */
const EXPORT_KEYWORDS = {
  material:['菌株','宿主','载体','质粒','模板','引物','探针','培养基','片段','插入','抗性','蛋白','酶','感受态','基因','链','细胞'],
  method:['处理','条件','程序','方式','体系','时间','体积','温度','转速','诱导','裂解','纯化','洗脱','退火','反应','上样','预混','稀释'],
  result:['结果','现象','产量','浓度','纯度','效率','Cq','OD','读数','检测','评价','备注']
};
